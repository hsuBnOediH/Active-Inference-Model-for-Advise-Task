% sim = 1;
% 
% revision of Simple_Advice_Model_CMG where all participants first decide
% to choose advisor or not (T1) and then decide left or right (T2)


function [results] = Simple_Advice_Model_CMG_same_num_choices(task, MDP, params, sim)
...
%for block = 1:task.num_blocks
    ...
    for trial = 1:task.num_trials
        ...
        A{2}(:,:,trial) = zeros(2,2);
        action_probs(:,:,trial) = zeros(2,2); % first column is probability of choosing advisor (y/n) then the side (left/right) 
        Q(:,:,trial) = zeros(3,1);
...
% compute action values (negative EFE)
        ...
        choice_probs_between_advisor_and_bandits = spm_softmax(params.inv_temp*Q(:,tp,trial))';
        action_probs(1,1,trial) = choice_probs_between_advisor_and_bandits(1); % probability of choosing advisor
        action_probs(2,1,trial) = 1 - choice_probs_between_advisor_and_bandits(1); % probability of NOT choosing advisor

        % select actions
        % note that 1 corresponds to choosing advisor, 2 corresponds to
        % choosing left bandit, 3 corresponds to choosing right bandit.
        if sim == 1
            actions(trial,tp) = find(rand < cumsum(choice_probs_between_advisor_and_bandits(:,tp,trial)'),1);
        else
        ...
        if actions(trial,1) ~= 1
            ...
            action_probs(:,2,trial) = spm_softmax(params.inv_temp*Q(2:3,1,trial))'; % probability of choosing left/right bandit
        ...
        elseif actions(trial,1) == 1
            ...
            % compute action probabilities
            action_probs(:,tp,trial) = spm_softmax(params.inv_temp*Q(2:3,2,trial))';
