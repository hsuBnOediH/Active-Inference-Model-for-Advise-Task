...
for idx_block = 1:num_blocks
    ...
    if isequal(M.model,@Simple_Advice_Model_CMG)
    ...
    elseif isequal(M.model,@Simple_Advice_Model_CMG_same_num_choices)
        MDP  = Simple_Advice_Model_CMG_same_num_choices(task, MDP,params, 0);
        % everybody makes the choice of if to take advisor or not then left
        % or right
        for j = 1:block_size
            chose_advisor = actions{j}(2,1) == 2; 
            prob_choose_advisor = MDP.blockwise.action_probs(1,1,j); 
            L = L + log(chose_advisor*prob_choose_advisor + (1-chose_advisor)*(1-prob_choose_advisor) + eps);
            ...
            prob_choose_left = MDP.blockwise.action_probs(1,2,j); 
            %prob_choose_bandit = MDP(j).P(1,actions{j}(2,2),2); old model
            L = L + log(chose_left_bandit*prob_choose_left + (1-chose_left_bandit)*(1-prob_choose_left) + eps);
